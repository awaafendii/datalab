...
async function buildPDFDoc(fileName: string, sheets: ExportSheet[]) {
  ...
  doc.text(
    `Fichier : ${fileName}${multi ? `  •  ${sheets.length} feuilles` : ""}  •  Généré le ${new Date().toLocaleString("fr-FR")}`,
    marginX,
    y,
  );
  doc.setTextColor(0);
  ...
  if (multi) {
    doc.setFontSize(13);
    doc.text("Sommaire", marginX, y);
    autoTable(doc, {
      startY: y + 2,
      head: [["Feuille", "Lignes", "Colonnes", "Doublons retirés", "Valeurs manquantes"]],
      body: sheets.map((s) => [
        s.name,
  ...
  for (const s of sheets) {
    if (multi) {
      ...
      doc.text(`Feuille : ${s.name}`, marginX, y);
      doc.setTextColor(0);
      y += 10;
    }
    ...
    autoTable(doc, {
      ...
      body: [
        ["Lignes", String(s.analysis.profile.rowCount)],
        ["Colonnes", String(s.analysis.profile.columnCount)],
        ["Lignes en double", String(s.analysis.profile.duplicateRows)],
        ["Valeurs manquantes", String(s.analysis.profile.totalMissing)],
      ],
      theme: "striped",
      headStyles: { fillColor: [37, 99, 235] },
      styles: { fontSize: 9 },
    ...
    autoTable(doc, {
      ...
      body: s.analysis.profile.columns.map((c) => [
        c.name,
        c.type,
        String(c.count),
        String(c.missing),
    ...
    doc.setFontSize(13);
    doc.text(
      `Échantillon des données nettoyées (30 lignes)${multi ? ` — ${s.name}` : ""}`,
      marginX,
      y,
    );
    const cols = s.dataset.columns.slice(0, 8);
    autoTable(doc, {
      startY: y + 2,
      head: [cols],
      body: s.dataset.rows
